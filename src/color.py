...
class Color(object):
  ...
  def histogram(self, input, n_bin=n_bin, type=h_type, n_slice=n_slice, normalize=True):
    ...
    if isinstance(input, np.ndarray):  # examinate input type
      ...
      img = np.array(Image.open(input).convert('RGB').resize((224,224)))
      # print("Processing image: {} with shape {}".format(input, img.shape))
    ...
    # print("Histogram shape for {0}: {1}".format(input, hist.shape))
...
if __name__ == "__main__":
  ...
  print(data.iloc[0]['img'])

  # test normalize
  hist = color.histogram(data.iloc[0]['img'], type='global')
