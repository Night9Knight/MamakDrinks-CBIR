...
def infer(query, samples=None, db=None, sample_db_fn=None, depth=None, d_type='d1'):
  ...
  results = []
  # print("Q Shape:", q_hist.shape)
  for idx, sample in enumerate(samples):
    s_img, s_cls, s_hist = sample['img'], sample['cls'], sample['hist']
    # print("S Shape:", s_hist.shape)
    # print("S detail", s_img, s_cls)
    if q_img == s_img:
      continue
    results.append({
                    'img': s_img,
  ...
  # Get list of results retrieved that are relevant
  num_of_correct_retrieval = [result for result in results if result['cls'] == q_cls]

  # Get list of items that has the same class as query
  num_of_same_class = [result for result in samples if result['cls'] == q_cls and result['img']!=q_img]

  # Precision = # of correct retrievals / # of retrievals
  prec = len(num_of_correct_retrieval) / len(results)
  # Recall = # of correct retrievals / # of relevant results in database
  rec = len(num_of_correct_retrieval) / len(num_of_same_class)
...
def evaluate(db, sample_db_fn, depth=None, d_type='d1'):
  ...
  for query in samples:
    ap, _, _ = infer(query, samples=samples, depth=depth, d_type=d_type)
